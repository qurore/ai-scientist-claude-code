...
creating run directories, executing experiment scripts with capture/timeout, and
compiling LaTeX — so the workflow is reproducible and resumable.

Run them as modules from the repo root, e.g.::

    .venv/bin/python -m aisci.run new --slug my_study --topic "..."
    .venv/bin/python -m aisci.exec runs/<id> code/n1.py --timeout 1800
